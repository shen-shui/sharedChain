...
public class SeckillController {
    ...
    private String buildUserKey(Long userId, Long skuId) {
        return "seckill:order:" + userId + ":" + skuId;
    }
    ...
    public Result<String> doSeckill(@PathVariable Long skuId) {
        ...
        Boolean firstSeckill = redisTemplate.opsForValue().setIfAbsent(userKey, "1");
        if (!Boolean.TRUE.equals(firstSeckill)) {
            // 已经参与过本次秒杀
            return Result.build("请勿重复秒杀", ResultCodeEnum.DATA_ERROR);
        }
        ...
        Long left = redisTemplate.opsForValue().decrement(stockKey);
        if (left == null || left < 0) {
            // 回滚多减的库存
        ...
        return Result.build("秒杀请求已受理，正在排队中", ResultCodeEnum.SUCCESS);
    }
}
